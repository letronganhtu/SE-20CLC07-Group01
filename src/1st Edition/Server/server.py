...
def SignIn(data):
    def queryDatabase(username, password):
        ...
        query_command = "SELECT USERNAME, PASSWORD, AVATAR, COVER FROM ACCOUNT"
    ...
    isExisted, avatar, cover = queryDatabase(username, password)
    if isExisted == True:
        cursor, conn = openDatabase();
        query_command = "UPDATE ACCOUNT SET STATUS = 'ONLINE' WHERE USERNAME = '{}'".format(username);
        cursor.execute(query_command)
        conn.commit()
        return True, username, avatar, cover
    else:
        return False, username, avatar, cover
...
def viewDetailPost(data):
    ...
    # Get like
    query_command = "SELECT LIKEP.TYPEOFLIKE, COUNT(*) AS NUMBER FROM LIKEPOST LIKEP, POST P WHERE P.ID = LIKEP.POSTID AND P.ID = '{}' GROUP BY LIKEP.TYPEOFLIKE ORDER BY COUNT(*) DESC".format(postID)
    likes = []
    for row in cursor.execute(query_command):
        type_of_like = row[0]
        number = row[1]
    ...
    # Get my like
    query_command = "SELECT LIKEP.TYPEOFLIKE FROM LIKEPOST LIKEP, POST P WHERE LIKEP.POSTID = P.ID AND P.ID = '{}' AND LIKEP.STARTUSER = '{}'".format(postID, watchingUser)
    type_of_my_like = ''
    for row in cursor.execute(query_command):
        type_of_my_like = row[0]
    
    # Get if the user added to their "My Favorites"
    query_command = "SELECT FAV.* FROM MY_FAVORITE FAV, POST P WHERE P.ID = FAV.POSTID AND P.ID = '{}' AND FAV.STARTUSER = '{}'".format(postID, watchingUser)
...
class Server(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not(post_data.get("SignIn") is None):
            check, uname, avatar, cover = SignIn(post_data)
            data = {"SignIn": check, "uname": uname, "avatar": str(avatar), "cover": str(cover)}
            ...
            check, uname, avatar, cover = SignIn(post_data)
            data = {"ContinueAsGuest": check, "uname": uname, "avatar": str(avatar), "cover": str(cover)}
