...
function deleteRecipeName() {
    recipe_name = document.getElementById("recipe-name").innerHTML;
    if (recipe_name === 'Enter Recipe Name...') {
...
function uploadMultipleImages() {
    function getMultipleImages() {
        ...
        if (additional_images.length != 0)  {
            document.getElementById('notice-upload').innerHTML = `Uploading images...`;
            document.getElementById('progress-area').style.display = 'block';
            document.getElementById('progress-bar').style.width = '2px';
        }
        else document.getElementById('progress-area').style.display = 'none';
        ...
        for (var i = 0; i < additional_images.length; i++) {
            ...
            picture.addEventListener('load', function(event) {
                ...
                if (percentage < 100)
                    document.getElementById('notice-upload').innerHTML = `Uploading images... ${percentage.toFixed(2)}%`;
                else if (percentage >= 100){
                    document.getElementById('notice-upload').innerHTML = `Done... ${percentage.toFixed(2)}%`;
                }
    
                document.getElementById('progress-bar').style.width = 7 * percentage.toFixed(2) + 'px';
                count_image++;
            });
        }
        document.getElementById("upload-additional-images-button").removeEventListener('change', getMultipleImages);
    }

    document.getElementById("upload-additional-images-button").addEventListener('change', getMultipleImages, false);
...
function deleteThumbnail() {
    ...
    document.getElementById("post-button").style.opacity = 1;
    document.getElementById("post-button").style.cursor = "no-drop";
}
...
function post() {
    ...
    req.addEventListener('load', function(){
        ...
        else if (received_data.PostRecipe == false) {
            alert("Fail to post because this account has been suspended!");
