...
def deletePost(data):
    ...
    query_command = "UPDATE POST SET AVAILABLE = 0 WHERE ID = '{}'".format(postID)
    cursor.execute(query_command)
    conn.commit()
...
def viewProfile(data):
    username = data['username']
    cursor, conn = openDatabase()
    query_command = "SELECT USERNAME, SURNAME + ' ' + FIRSTNAME AS FULLNAME, AVATAR FROM ACCOUNT WHERE USERNAME = '{}'".format(username)
    for row in cursor.execute(query_command):
        username = row[0]
        fullname = row[1]
        avatar = str(process_image_from_database(row[2]))
...
def getMyRecipes(data):
    username = data['username']
    cursor, conn = openDatabase()
    query_command = "SELECT ID, RECIPENAME, THUMBNAIL, CALO.AMOUNT FROM POST P, CALORIES CALO WHERE P.AVAILABLE = 1 AND P.ID = CALO.POSTID AND P.POSTINGUSER = '{}' ORDER BY P.POSTINGTIME DESC".format(username)
    posts = []
    for row in cursor.execute(query_command):
        postID = row[0]
        recipeName = row[1]
