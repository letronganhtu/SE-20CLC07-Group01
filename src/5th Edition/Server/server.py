...
def getReportedPosts():
    cursor, conn = openDatabase()
    query_command = "SELECT ID, POSTID, REPORTINGUSER, REASON_TEXT FROM REPORTED_POST ORDER BY REPORTINGTIME DESC"
    reportedPosts = []
    for row in cursor.execute(query_command):
        reportID = row[0]
        postID = row[1]
...
def viewProfile(data):
    username = data['username']
    cursor, conn = openDatabase()
    query_command = "SELECT USERNAME, SURNAME + ' ' + FIRSTNAME AS FULLNAME, AVATAR FROM ACCOUNT WHERE USERNAME = '{}' AND AVAILABLE = 1".format(username)
    for row in cursor.execute(query_command):
        username = row[0]
        fullname = row[1]
        avatar = str(process_image_from_database(row[2]))
        return True, username, fullname, avatar

def getFavoriteRecipes(data):
    startUser = data['username']
    cursor, conn = openDatabase()
    query_command = "SELECT FAV.POSTID, P.POSTINGUSER, ACC.SURNAME + ' '+ ACC.FIRSTNAME AS FULLNAME, ACC.AVATAR, P.RECIPENAME, CALO.AMOUNT, P.THUMBNAIL FROM MY_FAVORITE FAV, POST P, ACCOUNT ACC, CALORIES CALO WHERE FAV.STARTUSER = '{}' AND P.ID = FAV.POSTID AND P.AVAILABLE = 1 AND P.POSTINGUSER = ACC.USERNAME AND P.ID = CALO.POSTID ORDER BY FAV.CREATEDTIME DESC".format(startUser)
    posts = []
    for row in cursor.execute(query_command):
        postID = row[0]
        postingUser = row[1]
        ...
        posts.append([postID, postingUser, fullname, avatar, recipeName, calories, thumbnail])
    return True, posts

def getMyRecipes(data):
    username = data['username']
    cursor, conn = openDatabase()
    query_command = "SELECT ID, RECIPENAME, THUMBNAIL, CALO.AMOUNT FROM POST P, CALORIES CALO, ACCOUNT ACC WHERE P.AVAILABLE = 1 AND ACC.AVAILABLE = 1 AND P.POSTINGUSER = ACC.USERNAME AND P.ID = CALO.POSTID AND P.POSTINGUSER = '{}' ORDER BY P.POSTINGTIME DESC".format(username)
    posts = []
    for row in cursor.execute(query_command):
        postID = row[0]
        recipeName = row[1]
        ...
        calories = row[3]
        posts.append([postID, recipeName, thumbnail, calories])
    return True, posts
...
def changePassword(data):
    ...
    cursor, conn = openDatabase()
    query_command = "SELECT USERNAME, PASSWORD FROM ACCOUNT WHERE USERNAME = '{}' AND PASSWORD = '{}'".format(username, oldPassword)
    account = []
    for row in cursor.execute(query_command):
    ...
    if not account:
        return False
    query_command = "UPDATE ACCOUNT SET PASSWORD = '{}' WHERE PASSWORD = '{}'".format(newPassword, oldPassword)
    cursor.execute(query_command)
    conn.commit()
    return True
...
class Server(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not(post_data.get("SignIn") is None):
            ...
            check, username, fullname, avatar = viewProfile(post_data)
            data = {"viewProfile": check, "username": username, "fullname": fullname, "avatar": avatar}
