...
USE FOODAHOLIC_MANAGEMENT
GO
DECLARE @USERNAME VARCHAR(20) = 'ngthiennhan2002'
SELECT ID, RECIPENAME, THUMBNAIL
FROM POST P, CALORIES CALO
WHERE P.AVAILABLE = 1
AND P.ID = CALO.POSTID
AND P.POSTINGUSER = @USERNAME

DECLARE @STARTUSER VARCHAR(20)= 'ngthiennhan2002'
SELECT FAV.POSTID, P.POSTINGUSER, ACC.SURNAME + ' '+ ACC.FIRSTNAME AS FULLNAME, ACC.AVATAR, P.RECIPENAME, CALO.AMOUNT, P.THUMBNAIL
FROM MY_FAVORITE FAV, POST P, ACCOUNT ACC, CALORIES CALO
WHERE FAV.STARTUSER = @STARTUSER
AND P.ID = FAV.POSTID
AND P.AVAILABLE = 1
AND P.POSTINGUSER = ACC.USERNAME
AND P.ID = CALO.POSTID
ORDER BY FAV.CREATEDTIME DESC
...
SELECT LIKEP.TYPEOFLIKE, COUNT(*) AS NUMBER 
FROM LIKEPOST LIKEP, POST P, ACCOUNT ACC 
WHERE P.ID = LIKEP.POSTID 
AND P.ID = '1007' 
AND LIKEP.TYPEOFLIKE != 'NULL' 
AND P.AVAILABLE = 1 
AND ACC.AVAILABLE = 1
AND LIKEP.STARTUSER = ACC.USERNAME 
GROUP BY LIKEP.TYPEOFLIKE
ORDER BY COUNT(*) DESC

SELECT *
FROM LIKEPOST LIKEP
WHERE (LIKEP.TYPEOFLIKE = 'HAHA'
OR LIKEP.TYPEOFLIKE = 'LOVE')
AND LIKEP.POSTID = 1006

SELECT LIKEP.TYPEOFLIKE, COUNT(*) AS NUMBER FROM LIKEPOST LIKEP, POST P, ACCOUNT ACC WHERE P.ID = LIKEP.POSTID AND P.ID = '1006' AND LIKEP.TYPEOFLIKE != 'NULL' AND P.AVAILABLE = 1 AND ACC.AVAILABLE = 1 AND LIKEP.STARTUSER = ACC.USERNAME GROUP BY LIKEP.TYPEOFLIKE ORDER BY COUNT(*) DESC


--SELECT 4 POSTS WITH MOST LIKES AND HAHA--
SELECT TOP 4 P.ID, COUNT(*) AS AMOUNT
FROM POST P, ACCOUNT ACC, LIKEPOST LIKEP
WHERE P.POSTINGUSER = ACC.USERNAME AND P.AVAILABLE = 1 AND ACC.AVAILABLE = 1 
AND P.ID = LIKEP.POSTID
AND (LIKEP.TYPEOFLIKE = 'LOVE' OR LIKEP.TYPEOFLIKE = 'HAHA')
GROUP BY P.ID, P.POSTINGTIME
ORDER BY COUNT(*) DESC, P.POSTINGTIME DESC

--GET SPECIFIC TYPES OF LIKE OF 4 POSTS--
SELECT P.ID, LIKEP.TYPEOFLIKE, COUNT(*) AS NUMBER
FROM POST P, ACCOUNT ACC, LIKEPOST LIKEP
WHERE P.POSTINGUSER = ACC.USERNAME AND P.AVAILABLE = 1 AND ACC.AVAILABLE = 1 
AND P.ID = LIKEP.POSTID AND P.ID = 1006
GROUP BY P.ID, LIKEP.TYPEOFLIKE

--GET ALL OTHER INFORMATION--
SELECT P.ID, P.POSTINGUSER, P.RECIPENAME, ACC.AVATAR, P.THUMBNAIL 
FROM POST P, ACCOUNT ACC 
WHERE P.POSTINGUSER = ACC.USERNAME AND P.AVAILABLE = 1 AND ACC.AVAILABLE = 1
AND P.ID BETWEEN 1004 AND 1007