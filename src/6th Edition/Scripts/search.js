...
function showSearchResults() {
    ...
    req.addEventListener('load', function(){
        ...
        if (received_data.search == true) {
            ...
            document.getElementById('no-recipe-label').innerHTML = `<strong>Oops... no results for "${inputData}"</strong>`;
...
function showSortFilter() {
    ...
    document.getElementById('sort-by-fa-fa-icon').className = 'fa fa-chevron-up';
}

function hideSortFilter() {
    document.getElementById("sort-features-div").style.display = "none";
    document.getElementById('sort-by-fa-fa-icon').className = 'fa fa-chevron-down';
}

function sort(sortContent) {
    document.getElementById('loader-wrapper').style.display = 'block';
    let input = sessionStorage.getItem('input');
    let data = JSON.stringify(
        {"sort": "True", 'input': input, 'sortContent': sortContent}
    );
    ...
    req.addEventListener('load', function(){
        received_data = JSON.parse(req.responseText);
        if (received_data.sort == true) {
            posts = received_data.posts;
            let inputData = sessionStorage.getItem("input");
            document.getElementById('sort-by-title').innerHTML = `<u>Sort by:</u>&nbsp<i>${sortContent}</i><br>`;

            document.getElementById("input-data").innerHTML = '<strong>' + inputData + '</strong>';
            document.getElementById('recipes-div').innerHTML = ''; // RESET THE RECIPES DIV
            if (posts.length === 0 || posts.length === 1)
                document.getElementById("number-of-post").innerHTML = '(' + String(posts.length) + ' result)' ;
            else
                document.getElementById("number-of-post").innerHTML = '(' + String(posts.length) + ' results)' ;
